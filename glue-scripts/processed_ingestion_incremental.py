...
class ProcessedIncrementalDataSink(object):
    ...
    def write_target_data_jdbc(self, df):
        ...
        delete_join_clause=""
        print(composite_key)
        post_delete_query="begin;delete from {}  using {}  where ".format(self.params["catalog_table"],self.params["stg_table"])
        for key in composite_key:
            delete_join_clause += self.params["catalog_table"]+"."+key+"="+self.params["stg_table"]+"."+key+" and "
        delete_join_clause +=  "1=1;"
        insert_stg_query = "insert into {} select * from {};commit;truncate table {};".format(self.params["catalog_table"],self.params["stg_table"],self.params["stg_table"])
        post_query=post_delete_query+delete_join_clause+insert_stg_query+"end;"
        print(post_query)
        datasink1 = glueContext.write_dynamic_frame.from_jdbc_conf(frame = dyf, catalog_connection = self.params["glue_conn_name"], connection_options = {"dbtable": self.params["stg_table"], "database": self.params["catalog_db"], "postactions":post_query}, redshift_tmp_dir = self.params["output_tmp_path"], transformation_ctx = "datasink1")
