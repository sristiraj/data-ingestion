...
update_date,
update_userid,
max(cast(participant_firstname varchar(100))) as participant_firstname,
max(cast(participant_lastname varchar(100))) as participant_lastname,
max(case(participant_middlename varchar(100))) as participant_middlename,
...
udp.message_body _create_globalpersonidentifer ,
...
sha2(concat(udp.message_body_create_globalpersonidentifer,concat(rt.tbamessage_header_platforminternalid,rt.tbamessage_header_nationaltaxid))),256) as hash_kay,
...
case tbwabcoc.tbamessage_benefitsworkerattributes_val_benefitscurrentorganizationcodes_val_organizationid
WHEN '8357' THEN tbwabcoc.tbwabcoc.tbamessage_benefitsworkerattributes_val_benefitscurrentorganizationcodes_val_value
...
to_timestamp(case when rt.tbamessage_header_sourcesystemextractiontimestamp="" then null else rt.tbamessage_header_sourcesystemextractiontimestamp end ,"yyyy-MM-dd'T'HH:mm:ss.SSSz") message_timestamp,
...
prod_alight_trusted_tba.tba_datachangeevents_root_tbamessage_benefitworkerattributes_val_benefitcurrentorganizationcodes tbwabcoc 
on tbwa.tbamessage_benefitsworkerattributes_val_benefitscurrentorganizationcodes = tbwabococ.id and 
...
left outer join prod_alight_trusted_tba.tba_datachangeevents_root_tbamessage_benefitworkerattributes_val_benefitcurrentemploymentstatuses  tbwabcs 
on tbwa.tbamessage_benefitsworkerattributes_val_benefitcurrentemploymentstatuses = tbwabcs.id 
and tbwa.partition_load_dt_tmstmp = tbwabcs.partition_load_dt_tmstmp
left outer join 
udpdce idm on rt.tbamessage_header_platforminternalid = 
idm.message_body_create_idmapping_val_platforminternalid
...
udp.message_header_action,
udp.message_body_update_after_globalpersonidentifer,
...
where partition_load_dt_tmstmp=(select max(partition_load_dt_tmstmp) 
from prod_alight_trusted_tba.tba_udpdatachangeevents_root_message_body_update_after_modificationhistory)  ) idm 
left outer join 
(select * from prod_alight_trusted_tba.tba_udpdatachangeevents_root where partition_load_dt_tmstmp=(select max(partition_load_dt_tmstmp) from
 prod_alight_trusted_tba.tba_udpdatachangeevents_root)  ) udp 
...
where rn=1)
select distinct rt.tbamessage_header_platforminternalid as participant_id,
udp.message_body_update_after_globalpersonidentifer as global_participant_id,
current_date as record_start_date,
to_date('9999-09-09','yyyy-MM-dd') as record_end_date,
cast(1 as long) as current_indicator,
sha2(concat(udp.message_body_update_after_globalpersonidentifer,concat(rt.tbamessage_header_platforminternalid,rt.tbamessage_header_nationaltaxid))),256) as hash_kay,
current_date create_date,
current_date as update_date,
'glue' as update_userid,
rt.tbamessage_person_lastname as participant_lastname,
...
WHEN '21' THEN tbwabcs.tbamessage_benefitsworkerattributes_val_benefitscurrentemploymentstatuses_val_begindate 
WHEN '22' THEN tbwabcs.tbamessage_benefitsworkerattributes_val_benefitscurrentemploymentstatuses_val_begindate ELSE null END employment_cd_date,
to_timestamp(case when udp.tbamessage_header_messagetimestamp="" then null else udp.tbamessage_header_messagetimestamp end ,"yyyy-MM-dd'T'HH:mm:ss.SSSz") message_timestamp,
tbapbar.tbamessage_personbenefitaccountrelationships_val_originatingpersoninternalid originatingpersoninternalid
...
on rt.tbamessage_benefitworkerattributes = tbwa.id 
and rt.partition_load_dt_tmstmp = tbwa.partition_load_dt_tmstmp left outer join 
prod_alight_trusted_tba.tba_datachangeevents_root_tbamessage_benefitworkerattributes_val_benefitcurrentorganizationcodes tbwabcoc 
on tbwa.tbamessage_benefitsworkerattributes_val_benefitscurrentorganizationcodes = tbwabococ.id 
